...
const NotificationCard = ({ data, index, notificatonState }) => {
  ...
  let { notifications, notifications: { results, totalDocs }, setNotifications } = notificatonState;
  ...
  return (
    <div className={"p-6 border border-grey border-l-black " + (!seen ? "border-l-2" : "")}>
      <div className="flex gap-5 mb-3">
        ...
        <div className="w-full">
          <h1 className="font-medium text-xl text-dark-grey">
            ...
            <Link to={`/user/${username}`} className="mx-2 text-black underline">@{username}</Link>
          ...
          {
            type == "reply" ? <div className="p-4 mt-4 rounded-md bg-grey">
              <p>{replied_on_comment.comment}</p>
            </div> : <Link to={`/blog/${blog_id}`} className="font-medium text-dark-grey hover:underline line-clamp-1">{`"${title}"`}</Link>
          }
      ...
      <div className="flex gap-3 ml-14 pl-5 mt-3 text-dark-grey">
...

      {
        isReplying ?
          <div className="mt-8">
            <NotificationCommentField _id={_id} blog_author={user} index={index} replyingTo={comment._id} setReplying={setReplying} notification_id={notification_id} notificationData={notificatonState} />
          </div> : ""
      }
