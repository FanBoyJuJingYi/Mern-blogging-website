...
const PublishFrom = () => {
  ...
  const publishBlog = (e) => {
    ...
    if (!des.length || des.length > characterLimit) {
      return toast.error(`Write a description about your blog withing ${characterLimit} charaters to publish`);
    }
    if (!tags.length) {
      return toast.error("Add 1 tags to your blog post");
    }
    let loadingToast = toast.loading("Publishing...");
    e.target.classList.add("disable");
    let blogOBJ = { title, des, tags, banner, content, draft: false };
    ...
    <AnimationWrapper>
      <section className="w-screen min-h-screen grid items-center lg:grid-cols-2 py-16 lg:gap-4">
        ...
        <div className="max-w-[550px] center">
          ...
          <p className="font-gelasio text-xl leading-7 mt-4 line-clamp-2 break-all">{des}</p>

        </div>

        <div className="border-grey lg:border-1 lg:pl-8">

          <p className="text-dark-grey mb-2 mt-9">Blog Tille</p>
          ...
          ></textarea>
          <p className="mt-1 mb-4 text-dark-grey text-sm text-right">{characterLimit - des.length} charaters left</p>
