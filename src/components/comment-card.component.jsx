...
const CommentCard = ({ index, leftVal, commentData }) => {
  ...
  const [isReplying, setRelying] = useState(false);
  ...
  const handleReplyClick = () => {
    if (!access_token) {
      return toast.error("Login frist to leave a reply ...")
    }
    setRelying(preVal => !preVal);
  }
  ...
  return (
    <div className="w-full" style={{ paddingLeft: `${leftVal * 10}px` }}>
      <div className="my-5 p-6 rounded-sm border border-grey">
        ...
        <div className="flex gap-5 items-center mt-5">
          {
            commentData.isReplyLoaded ?
              <button className="text-dark-grey p-2 px-3 hover:bg-grey/30 rounded-md flex items-center gap-2" onClick={hideReplies}>
                <i className="fi fi-rs-comment-dots"></i> Hide Rely
              </button>
        ...
        </div>
        {
          isReplying ?
            <div className="mt-8">
              <CommentField action="reply" index={index} replyingTo={_id} setReplying={setRelying} />
            </div>
            : ""
        }
